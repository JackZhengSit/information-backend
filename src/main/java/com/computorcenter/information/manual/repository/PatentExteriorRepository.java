...
public interface PatentExteriorRepository extends JpaRepository<PatentExterior, Long> {
    ...
    @Query(
            value = "select * from patent_exterior "
                    + "where ( ordernum is null or ordernum between :orderNumStart and :orderNumEnd)"

                    + " and (inventionTitile is null or inventionTitile like %:inventionTitile%)"
                    + " and (application_no is null or application_no like %:applicationNo%)"
                    + " and (application_day is null or application_day like %:applicationDay%)"
                    + " and (publication_no is null or publication_no like %:publicationNo%)"
                    + " and (publication_day is null or publication_day like %:publicationDay%)"
                    ...
                    + " and (priority_day is null or priority_day like %:priorityDay%)"
                    + " and (mainClass_num is null or mainClass_num like %:mainClassNum%)"
    ...
    public Page<PatentExterior> manageSearch(
            ...
            @Param("applicationDay") String applicationDay,
            ...
            @Param("priorityDay") String priorityDay,
    ...
    @Query(
            value =
                    "update patent_exterior set file_url =:fileUrl ,file_name=:filename where id = :id",
            nativeQuery = true)
